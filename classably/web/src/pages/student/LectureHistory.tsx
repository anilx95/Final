  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-100 flex items-center gap-2.5 tracking-tight">
            <Calendar className="w-5 h-5 text-sky-400" /> Lecture Archive & Recordings
          </h1>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#1b2538] pb-3">
                <div>
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <h3 className="font-bold text-slate-100 text-sm sm:text-base tracking-tight">{s.subject || 'Lecture Session'}</h3>
                  </div>
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-300 mt-1.5 flex-wrap">
                    ...
                    <span className="flex items-center gap-1 text-indigo-300 font-semibold">
                      <BookOpen className="w-3.5 h-3.5 text-indigo-400" /> Topic: {s.topic || 'General Lecture'}
                    </span>
                  </div>
                </div>

                <div className="flex items-center gap-2.5 self-start sm:self-auto">
                  <div className="text-xs text-slate-400 font-mono flex items-center gap-1.5 bg-[#080c14] px-3 py-1.5 rounded-lg border border-[#1b2538]">
      ...
      {activeModalSession && (
        ...
        >
          {/* View Tab Selector */}
          <div className="flex items-center gap-2 pb-3 border-b border-[#1b2538] text-xs font-semibold overflow-x-auto">
            <button
          ...
          <div className="py-4 space-y-4">
            {activeTab === 'video' && (
              <div className="space-y-2.5">
                <div className="relative rounded-xl overflow-hidden bg-black border border-[#1b2538] shadow-2xl flex items-center justify-center">
                  <video
            ...
            {activeTab === 'audio' && (
              <div className="space-y-4 p-6 bg-[#080c14] rounded-xl border border-[#1b2538] text-center">
                <div className="p-3.5 rounded-xl bg-emerald-500/10 text-emerald-400 w-fit mx-auto border border-emerald-500/20">
                  <Volume2 className="w-7 h-7 animate-pulse" />
                </div>
                <div>
                  <h3 className="font-bold text-slate-100 text-sm">Full Lecture Audio Track</h3>
                  <p className="text-xs text-slate-400 mt-1">High-clarity audio recording for accessible playback</p>
                </div>
                ...
                {subLoading ? (
                  ...
                  <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
                    {(filteredSubtitles || []).map((sub, i) => (
                      <div key={i} className="p-2.5 rounded-xl bg-[#080c14] border border-[#1b2538] text-xs flex items-start gap-2.5">
                        <span className="font-mono text-[10px] text-sky-400 font-bold bg-sky-500/10 px-2 py-0.5 rounded shrink-0 border border-sky-500/20">
                          {sub.timestamp || `#${i + 1}`}
                        </span>
            ...
            {activeTab === 'summary' && summary && (
              <Card variant="ai" className="p-5 space-y-3.5">
                <div className="flex items-center gap-2 border-b border-[#1b2538] pb-2">
                  <Sparkles className="w-4 h-4 text-amber-400" />
                  <h3 className="font-bold text-slate-100 text-xs sm:text-sm tracking-tight">AI Key Takeaways</h3>
                </div>

                {((summary.key_takeaways || []).length > 0) && (
                  <div className="space-y-1.5">
                    <h4 className="text-[11px] font-bold text-slate-300 uppercase tracking-wider font-mono">Key Concepts</h4>
                ...
                {summary.full_summary && (
                  <div className="space-y-1 pt-2 border-t border-[#1b2538]">
                    <h4 className="text-[11px] font-bold text-slate-300 uppercase tracking-wider font-mono">Overview</h4>
          ...
          <div className="pt-3 border-t border-[#1b2538] flex flex-wrap items-center justify-between gap-2 text-xs">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <a href={exportApi.downloadAudioUrl(activeModalSession.id)} download className="btn-secondary text-xs py-1.5">
                <Volume2 className="w-3.5 h-3.5 text-emerald-400" /> MP3 Audio
              </a>
